...
public class SimpleDinosaurController : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    [Header("🌐 INTEREST MANAGEMENT SYSTEM")]
    ...
    // Para detectar fantasmas/objetos sin actualización
    ...
    void Start()
    {
        ...
        // 🌐 INTEREST MANAGEMENT: Configurar grupos de interés
    ...
    void Update()
    {
		...
		// 🌐 INTEREST MANAGEMENT: Actualizar grupo de interés cada 2 segundos
...
void UpdateTimers()
{
	...
	public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
	{
		...
		else
		{
			...
			try
			{
				...
				// 9. MARCAR actualización de red (para detectar fantasmas)
				lastNetworkUpdate = Time.time;
				lastKnownPosition = networkPosition;
			}
	...
	/// Este sistema divide el mundo en un grid 16x16 (256 celdas totales)
	/// y solo sincroniza objetos en celdas cercanas, reduciendo el tráfico de red
	/// </summary>
	...
	/// Convierte coordenadas del mundo a coordenadas de grid 16x16
	/// </summary>
	...
	/// Esto permite que los jugadores vean objetos en celdas vecinas
	/// </summary>
	...
	/// <summary>
	/// Configuración inicial del Interest Group con delay
	/// Se ejecuta después de que la red se estabilice
	/// </summary>
	System.Collections.IEnumerator DelayedInterestGroupSetup()
	{
	...
	void DebugInterestGroups()
	{
		...
		{
			Debug.LogWarning("No estás en una sala de Photon");
			return;
		}

		byte currentGroup = CalculateInterestGroup(transform.position);
		byte[] adjacentGroups = GetAdjacentGroups(currentGroup);

		Debug.Log($"=== INTEREST GROUPS DEBUG ===");
		Debug.Log($"Posición: {transform.position}");
		Debug.Log($"Grupo actual: {currentGroup}");
		Debug.Log($"Grupos adyacentes: {string.Join(", ", adjacentGroups)}");
		Debug.Log($"Grid Cell Size: {gridCellSize}m");
		Debug.Log($"Adjacent Radius: {adjacentRadius}");
	}
	#endregion
}
